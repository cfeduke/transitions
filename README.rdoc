 ...
 class Product
   ...
   state_machine do
     ...
     state :out_of_stock, :exit => :exit_out_of_stock
		 state :on_order, :enter => [:notify_customers, :process_pending_orders]
     ...
     event :available do
       transitions :to => :available, :from => [:out_of_stock, :on_order], :guard => lambda { |product| product.in_stock > 0 }
     end
   end

   private
   def exit_out_of_stock; end
	 def notify_customers; end
	 def process_pending_orders; end
 end
